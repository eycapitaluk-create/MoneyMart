...
const FundDetailPage = () => {
  const { id } = useParams(); // URL에서 넘어온 ID (예: QF001131)
  ...
  useEffect(() => {
    // ✅ 수정됨: URL의 id와 데이터의 fund.id를 비교해서 찾습니다.
    const foundFund = funds.find(f => f.id === id);
    setFund(foundFund);
  }, [id]);
  ...
  if (!fund) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-gray-500">
        ...
        <p>ファンド情報を読み込んでいます... (ID: {id})</p>
        <button onClick={() => navigate('/fund')} className="mt-4 text-blue-500 underline">
          リストに戻る
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12 animate-fadeIn font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="bg-white rounded-b-xl p-6 shadow-sm border border-t-0 border-gray-200 min-h-[400px]">
             ...
             {activeTab === 'chart' && (
                 <div className="h-80 bg-gray-50 rounded-lg flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 animate-fadeIn">
                     ...
                     <p className="text-xs">リアルタイムデータの連携が必要です。</p>
                 </div>
             )}
        ...
        <div className="lg:col-span-1">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-blue-100 sticky top-28">
