        ...
        <div className="absolute bottom-20 right-0 w-[90vw] md:w-96 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col transition-all duration-300 animate-slideUp origin-bottom-right h-[600px] max-h-[80vh]">
          ...
          <div className="bg-slate-900 dark:bg-slate-800 p-4 flex justify-between items-center text-white shrink-0">
            <div className="flex items-center gap-3">
              {mode === 'contact' ? (
                <button onClick={() => setMode('chat')} className="hover:bg-white/10 p-1 rounded-full transition"><ChevronLeft/></button>
              ) : (
          ...
          {/* Body */}
          <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 p-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700">
          ...
          {/* Footer (Input Area) - Chat Mode Only */}
          {mode === 'chat' && (
            <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
              ...
              <div className="text-center mt-2">
                <p className="text-[10px] text-slate-400">Powered by MoneyMart AI</p>
              </div>
            </div>
          )}
        </div>
